...
import es.noa.rad.game.engine.core.component.GamePanel;
  ...
  public class UserInterface {
     ...
     *
     */
    public static final int MAX_MESSAGE_ALIVE = 120;
    ...
    public final void draw(
      ...
      if (this.messageActive) {
        _graphics2D.setFont(_graphics2D.getFont().deriveFont(60.0F));
        _graphics2D.setColor(Color.black);

        /* Measure the actual width of the text with the active font. */
        final FontMetrics fontMetrics = _graphics2D.getFontMetrics();
        final int textWidth = fontMetrics.stringWidth(this.message);
        final int textAscent = fontMetrics.getAscent();

        /*
         * Centering horizontally and vertically places it a quarter of the way
         * down.
         */
        final int messageX
          = ((this.gamePanel.getScreenWidth() - textWidth) / 2);
        final int messageY
          = (((this.gamePanel.getScreenHeight() + textAscent) / 4) * 3);
        ...
        this.messageAlive++;
        if (this.messageAlive > UserInterface.MAX_MESSAGE_ALIVE) {
          this.messageAlive = 0;
          this.messageActive = false;
        } 
      }
